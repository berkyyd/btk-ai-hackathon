...
export async function POST(request: NextRequest) {
  try {
    ...
    // Soruları doğru formata dönüştür (question -> text)
    const formattedQuestions = questions.map((q: any) => ({
      id: q.id,
      text: q.question || q.text, // question alanını text'e dönüştür
      type: q.type,
      options: q.options,
        ...
        quizResults.forEach(result => {
          result.answers?.forEach(answer => {
            if (!answer.isCorrect) {
              ...
              if (question) {
                wrongAnswers.push({
                  question: question.text || question.question, // Her iki alanı da kontrol et
