...
interface QuizResult {
  ...
  questions: Array<{
    id: string;
    text: string;
    question?: string; // Eski format için geriye uyumluluk
    topic?: string;
    correctAnswer: string;
    explanation?: string;
  }>;
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Soruları doğru formata dönüştür ve undefined değerleri temizle
    const formattedQuestions = questions.map((q: any) => ({
      ...
      topic: q.topic || 'Genel'
    })).filter((q: any) => q.id && q.text); // Boş soruları filtrele

    // Cevapları temizle ve undefined değerleri kaldır - boş cevapları da dahil et
    const cleanedAnswers = answers.map((answer: any) => ({
      questionId: answer.questionId || '',
      userAnswer: answer.userAnswer || '', // Boş cevap da kaydedilsin
      correctAnswer: answer.correctAnswer || '',
      isCorrect: answer.isCorrect || false
    })).filter((a: any) => a.questionId); // Sadece questionId varsa kaydet, userAnswer boş olabilir

    // Quiz sonucunu kaydet - undefined değerleri temizle
    const quizData = {
      userId: userId || '',
      quizId: quizId || '',
      score: score || 0,
    ...
    // undefined değerleri temizle ve null değerleri de kontrol et
      ...
      Object.entries(quizData)
        ...
        .map(([key, value]) => {
          ...
          if (Array.isArray(value)) {
            return [key, value.filter(item => item !== undefined && item !== null)];
          }
          // Object'leri temizle
    ...
    const docRef = await addDoc(collection(db, 'quizResults'), cleanQuizData);
    ...
    try {
      ...
      try {
        ...
        // Composite index eksik, fallback kullanılıyor...
        ...
        quizResults.forEach(result => {
          result.answers?.forEach(answer => {
            if (!answer.isCorrect) {
              ...
              if (question) {
                wrongAnswers.push({
                  question: question.text, // Sadece text alanını kullan
