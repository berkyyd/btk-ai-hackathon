...
import { collection, query, where, getDocs, orderBy, addDoc } from 'firebase/firestore/lite';
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Yanlış cevapları ve boş cevapları analiz et
    const wrongAnswers: WrongAnswer[] = [];
    const emptyAnswers: WrongAnswer[] = [];
    ...
    quizResults.forEach(result => {
      result.answers?.forEach(answer => {
        ...
        if (question) {
          // Boş cevapları da dahil et
          if (!answer.userAnswer || (typeof answer.userAnswer === 'string' && answer.userAnswer.trim() === '')) {
            emptyAnswers.push({
...
${JSON.stringify(emptyAnswers.slice(0, 20), null, 2)}
...
`;
    ...
    // Analiz sonucunu Firestore'a kaydet - undefined değerleri temizle
    try {
      ...
      await addDoc(analyticsRef, cleanAnalyticsData);
    } catch (saveError) {
